for (var i = 0; i < GridResolution; i++) {
    for (var j = 0; j < GridResolution; j++) {
		...
		//spr_Grid.image_blend(c_aqua);
		draw_sprite_ext(spr_Grid,0,i*8,j*8,1,1,0,
		make_color_hsv(abs(grid[i][j].X),abs(grid[i][j].Y),
		abs(grid[i][j].Y)+abs(grid[i][j].X)*20)
		,1);
	
		//draw_text(i*8,j*8,grid[i][j].X);
